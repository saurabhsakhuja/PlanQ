...
import 'package:plan_q/src/modules/auth/presentation/screens/widgets/readblock_question_widget.dart';
import 'package:plan_q/src/modules/auth/presentation/screens/widgets/workout_frequency_question_widget.dart';
import 'package:plan_q/src/modules/auth/presentation/screens/widgets/workout_question_widget.dart';

typedef QuestionCallback = VoidCallback;
...
class _GeneralDetailFillupScreenState extends State<GeneralDetailFillupScreen> {
  int _currentQuestionIndex = 0;
  final PageController _pageController = PageController();
  late final List<Widget> _questions; // Declare as late final

  @override
  void initState() {
    super.initState();
    // Initialize _questions in initState, passing the callback
    _questions = [
      PersonalDescriptorQuestionWidget(onContinue: _nextQuestion),
      FitnessMissionQuestionWidget(onContinue: _nextQuestion),
      RoadblockQuestionWidget(onContinue: _nextQuestion),
      FitnessExperianceQuestionWidget(onContinue: _nextQuestion),
      ActivityLevelQuestionWidget(onContinue: _nextQuestion),
  ...
  void _nextQuestion() {
    if (_currentQuestionIndex < _questions.length - 1) {
      ...
      // Navigate to the next screen (e.g., Register screen)
      GoRouter.of(context).goNamed(AppRoutes.WORKOUTS_MAIN_SCREEN_ROUTE_NAME);
    }
  }
          ...
          child: SafeArea(
            child: Column(
              ...
              children: [
                ...
                // The CommonSubmitButton is now removed from here
              ],
            ),
          ),
        ),
